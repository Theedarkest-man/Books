<!-- This is code allow admin to add books -->
<?php
...
if(isset($_POST['add_product'])){
   ...
   $select_product_name = mysqli_query($conn, "SELECT name FROM `tblbooks` WHERE name = '$name'") or die('query failed');

   if(mysqli_num_rows($select_product_name) > 0){
      $message[] = 'product name already added';
   }else{
      $add_product_query = mysqli_query($conn, "INSERT INTO `tblbooks`(name, price, image) VALUES('$name', '$price', '$image')") or die('query failed');
...
if(isset($_GET['delete'])){
   ...
   $delete_image_query = mysqli_query($conn, "SELECT image FROM `tblbooks` WHERE id = '$delete_id'") or die('query failed');
